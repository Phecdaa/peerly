  ...
  return (
    <div className="bg-white min-h-screen md:rounded-3xl md:shadow-sm md:overflow-hidden md:border border-zinc-100">
      ...
      <div className="px-6 -mt-7 relative z-10 text-zinc-900">
        <form action="/explore" method="GET" className="bg-white rounded-xl shadow-md border border-zinc-100 flex items-center px-4 py-3 gap-3">
      ...
      <div className="px-6 mt-6">
        <div className="bg-slate-900 rounded-2xl overflow-hidden relative h-36 flex items-center px-6 shadow-sm">
          ...
          <div className="relative z-10 text-white">
              ...
              <Link href="/explore?subject=javascript" className="inline-block text-[10px] font-semibold bg-white text-blue-900 px-4 py-1.5 rounded-full shadow-sm hover:opacity-90 transition">Cari Mentor</Link>
          </div>
        </div>
      </div>
      ...
      <div className="px-6 mt-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {(courses ?? []).map((course) => (
             <Link key={course.id} href={`/explore?subject=${course.slug}`} className="relative h-32 md:h-40 rounded-2xl overflow-hidden group block shadow-sm border border-zinc-100">
