...
export function Navigation({
  ...
  const isApprovedMentor = isMentor && mentorStatus === "approved";

  // ── Build nav items based on mode ──
  let navItems: NavItem[] = [];

  if (mode === "mentor" && isApprovedMentor) {
    // Mentor Mode navigation
    navItems = [
      { label: "Mentor Home", href: "/mentor/earnings", icon: "monitoring" },
      { label: "Jadwal", href: "/mentor/availability", icon: "calendar_month" },
      { label: "My Rooms", href: "/rooms", icon: "groups" },
      { label: "Wallet", href: "/wallet", icon: "payments" },
    ];
  } else {
    // Student Mode navigation (default)
    navItems = [
  ...
  // Admin tab — always available in both modes for admins
  if (role === "admin") {
  ...
  // Extra items for mobile "More" menu (items beyond the first 4)
  const mobileMainItems = navItems.slice(0, 4);
  const mobileExtraItems: NavItem[] = navItems.slice(4);
  // Always add notifications to the more menu on mobile
  mobileExtraItems.unshift({ label: "Notifications", href: "/notifications", icon: "notifications" });
  ...
  const modeColor = mode === "mentor" ? "text-secondary" : "text-primary";
  const modeIcon = mode === "mentor" ? "psychology" : "school";
  const modeLabel = mode === "mentor" ? "Mentor Mode" : "Student Mode";
    ...
    <>
      ...
      <nav className="hidden md:flex fixed top-0 left-0 w-full justify-between items-center px-6 py-3 h-16 bg-surface-container-lowest/90 backdrop-blur-md border-b border-outline-variant/30 shadow-[0_4px_20px_rgba(0,0,0,0.02)] z-50">
        <div className="flex items-center gap-8">
          <Link href={mode === "mentor" ? "/mentor/earnings" : "/dashboard"} className="text-[24px] font-black text-primary tracking-tight font-sans">
            Peerly
          </Link>
          {/* Mode Indicator Badge */}
          {isApprovedMentor && (
            <div className={`hidden lg:flex items-center gap-1.5 px-3 py-1 rounded-full border ${mode === "mentor" ? "border-secondary-fixed-dim bg-secondary-container/30" : "border-primary-fixed-dim bg-primary-container/10"}`}>
              <span className={`material-symbols-outlined text-[16px] ${modeColor}`} style={{ fontVariationSettings: "'FILL' 1" }}>{modeIcon}</span>
              <span className={`font-label-sm text-label-sm ${modeColor} font-semibold`}>{modeLabel}</span>
            </div>
          )}
      ...
      <aside className="fixed left-0 top-16 bottom-0 w-64 py-8 px-4 flex-col gap-2 bg-surface-container-lowest border-r border-outline-variant/30 hidden lg:flex z-40">
        ...
        <div className="mt-auto pt-4 border-t border-outline-variant/30">
          {mode === "mentor" ? (
            <Link href="/mentor/availability" className="w-full flex items-center justify-center gap-2 bg-secondary text-on-secondary px-4 py-3 rounded-xl font-label-md font-semibold hover:bg-on-secondary-fixed-variant transition-colors shadow-[0_4px_14px_rgba(0,108,73,0.2)]">
